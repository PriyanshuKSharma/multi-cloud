...
\end{itemize}
The results demonstrate that the overhead introduced by Cloud Simplify (task queueing and module preparation) is negligible ($<$2 seconds) compared to the provider's execution time.

\subsection{System Throughput}
By leveraging Celery with multiple workers, we tested the platform's ability to handle concurrent provisioning. With 10 concurrent requests, the API response time remained below 100ms, proving the efficiency of the asynchronous architecture.
